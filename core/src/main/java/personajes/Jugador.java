...
public class Jugador extends Actor implements IdManager{
    ...
    public void actualizarDesdeServidor(float x, float y, boolean mirandoDerecha) {
        if (this.cuerpo != null) {
            // ✅ Validar que las coordenadas sean válidas
            if (Float.isNaN(x) || Float.isNaN(y) || Float.isInfinite(x) || Float.isInfinite(y)) {
                System.err.println("❌ Posición inválida recibida: (" + x + ", " + y + ")");
                return;
            }

            // ✅ CRÍTICO: Actualización DIRECTA sin interpolación
            // La interpolación causa retraso visual de ~300ms
            this.cuerpo.setTransform(x, y, this.cuerpo.getAngle());

            // ✅ También detener cualquier velocidad residual
    ...
    public void corregirPosicionSuave(float serverX, float serverY, boolean mirandoDerecha) {
        if (this.cuerpo == null) return;

        float currentX = this.cuerpo.getPosition().x;
        float currentY = this.cuerpo.getPosition().y;

        // Calcular diferencia
        float diffX = serverX - currentX;
        float diffY = serverY - currentY;
        float distance = (float) Math.sqrt(diffX * diffX + diffY * diffY);

        // ✅ Solo corregir si la diferencia es significativa (> 0.1 metros)
        if (distance > 0.1f) {
            // Si la diferencia es muy grande (> 1 metro), teleportar
            if (distance > 1.0f) {
                this.cuerpo.setTransform(serverX, serverY, this.cuerpo.getAngle());
                System.out.println("⚡ Corrección fuerte: " + distance + "m");
            } else {
                // Interpolación suave (10% hacia el servidor)
                float newX = currentX + diffX * 0.1f;
                float newY = currentY + diffY * 0.1f;
                this.cuerpo.setTransform(newX, newY, this.cuerpo.getAngle());
            }
        }
